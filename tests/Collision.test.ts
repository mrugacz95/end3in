...
import { Rectangle, Circle } from '../src/Body';
...
test('checks AABB collision detection when circle and rectangle colliding', () => {
    // given
    const b1 = new Circle(0.5, 1, false);
    b1.pos = new Vec2(0, 0);
    const b2 = new Rectangle(1, 1, TEST_MASS, true);
    b2.pos = new Vec2(-1, 0);
    b2.rot = Math.PI / 4;
...
test('checks AABB collision detection when two circles colliding', () => {
    ...
    const b1 = new Circle(0.5, 1, false);
    b1.pos = new Vec2(0, 0)
    const b2 = new Circle(0.5, 1, false);
    b2.pos =  new Vec2(0.99, 0.99)
...
test('checks SAT collision detection when two circles colliding', () => {
    ...
    const c1 = new Circle(circlesRadius, 1, false);
    c1.pos = new Vec2(0, 0);
    const overlap = 0.1;
